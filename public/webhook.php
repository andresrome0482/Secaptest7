...
if (!didit_verify_signature($rawBody, $signature, $webhookSecret)) {
    log_event('Firma de webhook inválida', ['signature_present' => (bool) $signature]);
...
log_event('Webhook recibido', ['status' => $status, 'vendor_data' => $vendorData, 'session_id' => $sessionId]);
